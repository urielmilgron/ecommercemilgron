...
import { addDoc, collection, getDocs, doc, query, where, documentId, writeBatch } from 'firebase/firestore' //Importamos addDoc (agg doc a firestore) y 
...
const Checkout = () => {
    ...
    const { cart, getQuantity, totalPriceProd, clearCart } = useContext(CartContext)
    ...
    const createOrder = async (data) => {
        ...
        try {
            ...
            if (noStock.length === 0) {
                ...
                return <main><h2 className='titleCart'>No se pudo realizar la compra ya que no hay stock en uno de los productos elegidos</h2></main>
            }
    ...
    if (isLoading) {
        return <main className='mainLoading'> <div className="loader"></div> <h2 className='titleCart'>Se está generando tu orden</h2></main>
    }
    if (orderCreated) {
       return <main><h2 className='titleCart'>Se ha generado tu orden con éxito, se te redireccionará al inicio en unos instantes! El número de orden lo recibiras en tu email en unos minutos, gracias por su compra!</h2></main>
    }
    return (
        <main>
        <div className="divForm">
            ...
            <form className='formCheck' onSubmit={handleSubmit(createOrder)}>
                <div className='divInput'>
                    ...
                    <input className='inputForm' type="text" {...register('firstName', {
                        required: true,
                        maxLength: 15
                    })} ></input>
                    {errors.firstName?.type === 'required' && <p className='obligatory'>El campo es requerido</p>}
                </div>
                <div className='divInput'>
                    <label className='labelForm'>Apellido</label>
                    <input className='inputForm' type="text" {...register('lastName',
                        {
                            required: true,
                            maxLength: 20
                        }
                    )} ></input>
                    {errors.lastName?.type === 'required' && <p className='obligatory'>El campo es requerido</p>}
                </div>
                <div className='divInput'>
                    ...
                    {errors.email?.type === 'required' && <p className='obligatory'>El campo es requerido</p>}
                    {errors.email?.type === 'pattern' && <p className='obligatory'>El formato del email no es válido</p>}
                </div>
                <div className='divInput'>
                    <label className='labelForm'>Teléfono</label>
                    <input className='inputForm' type="text" {...register('phone',
                        {
                            required: true,
                            maxLength: 30
                        }
                    )}></input>
                    {errors.phone?.type === 'required' && <p className='obligatory'>El campo es requerido</p>}
                </div>
                <div className='divInput'>
                    <label className='labelForm'>Dirección</label>
                    <input className='inputForm' type="text" {...register('address',
                        { required: true }
                    )} ></input>
                    {errors.address?.type === 'required' && <p className='obligatory'>El campo es requerido</p>}
                </div>
                <input className='ButtonsCategory createOrd' type="submit" value="Generar orden" />
            </form>
        </div>
        </main>
    )
